...
public class ArmMuscleHighlighter : MonoBehaviour
{
    [Serializable]
    ...
    private Queue<(long, float[])> receivedArmDataQueue = new Queue<(long, float[])>();
    ...
    [HideInInspector]
    public Queue<(long, float[])> armQueue = new Queue<(long, float[])>();
    [HideInInspector]
    public (long, float[]) armChannels = (0L, new float[8]);  // 최신 값 (여전히 유지)


    // Transform references for muscle visualization
    private Transform rightArmFlexion;
    private Transform rightArmExtension;
    ...
    // Butterworth Filters for 8 EMG channels
    private ButterworthFilter[] bandpassFilters;
    private float armFlexionEMA; // EMA smoothing
    private float armExtensionEMA; // EMA smoothing
    ...
    // Threshold damping
    public float alpha = 0.2f; // EMA smoothing factor
    public float threshold = 0.05f; // Min activation threshold
    ...
    // MVC 값을 저장하는 배열
    private float[] mvcValuesArm = new float[8];
    ...
    void Start()
    {
        ...
        rightArmFlexion = avatarTransform.Find("Hips/Spine/Spine1/Spine2/RightShoulder/RightArm/Muscle127");
        rightArmExtension = avatarTransform.Find("Hips/Spine/Spine1/Spine2/RightShoulder/RightArm/Muscle134");

        if (rightArmFlexion == null || rightArmExtension == null)
        {
            Debug.LogError("Failed to find the muscle Transform. Please check the hierarchy.");
        }

        // Initialize Butterworth filters & RMS buffers
        ...
        recordingData = new List<float>[8]; // 8개의 채널에 대해 기록 데이터 초기화
        mvcValues = new float[8]; // MVC 값을 초기화
        for (int i = 0; i < 8; i++)
        {
            recordingData[i] = new List<float>();
            mvcValues[i] = 0f; // 초기값은 0
        }

        string mvcArmFilePath = Path.Combine(mvcFilePath + SubjectName + "/", SubjectName + "Arm.csv");

        // MVC 값을 읽고 초기화
        mvcValuesArm = LoadMVCValuesFromFile(mvcArmFilePath);

        if (mvcValuesArm == null)
        {
    ...
    void Update()
    {
        if (rightArmFlexion != null && rightArmExtension != null)
        {
            ...
            RealtimeMuscle.DataSource.SetValue("Flexion", "Arm", armFlexionMuscleActivation);
            RealtimeMuscle.DataSource.SetValue("Extension", "Arm", armExtensionMuscleActivation);
        }
    }
            ...
            while (isRunning)
            {
                try
                {
                    ...
                    foreach (var (unixTime, emgBytes) in emgFrames)
                    {
                        ...
                        //Debug.Log("unix time " + unixTime + " emg bytes " + string.Join(", ", emgFloatValues));
                        UpdateMuscleActivations(emgFloatValues, unixTime);
                        armChannels = (unixTime, emgFloatValues); // 마지막 프레임 기준 저장

                        lock (armQueue)
                        {
                            armQueue.Enqueue((unixTime, emgFloatValues));
                        }
                        // ServerListen()의 foreach 루프에서
                    }
    ...
    private void UpdateMuscleActivations(float[] emgValues, long unixTime)
    {
        ...
        float normArmFlexion = GetMvcNormalizedFlexion(mvcNormalizedEmg);
        float normArmExtension = GetMvcNormalizedExtension(mvcNormalizedEmg);
        ...
        armFlexionMuscleActivation = EmgVisualizationMath.CapForDisplay(
            ApplyThresholdDamping(armFlexionEMA, armFlexionMuscleActivation, threshold));
        armExtensionMuscleActivation = EmgVisualizationMath.CapForDisplay(
            ApplyThresholdDamping(armExtensionEMA, armExtensionMuscleActivation, threshold));
    ...
    {
        ApplyMaterialToTransform(rightArmFlexion, highlightMaterial);
        ApplyMaterialToTransform(rightArmExtension, highlightMaterial);
    }

    // Visualize muscle activation dynamically
    void VisualizeMuscleActivation()
    {
        VisualizeMuscleActivationOnTransform(rightArmFlexion, armFlexionMuscleActivation);
        VisualizeMuscleActivationOnTransform(rightArmExtension, armExtensionMuscleActivation);
    }
    ...
    {
        return (armFlexionMuscleActivation, armExtensionMuscleActivation);
    }
    ...
    {
        isRecording = true;
        Debug.Log("MVC recording started.");
        for (int i = 0; i < 8; i++)
        {
            recordingData[i].Clear();
        }
    }
    ...
    {
        isRecording = false;
        Debug.Log("MVC recording stopped. Calculating MVC...");

        // 각 채널별 최대값 계산
        for (int i = 0; i < 8; i++)
        {
            if (recordingData[i].Count > 0)
            {
                mvcValues[i] = recordingData[i].Max();
            }
        }
    ...
    {
        // 저장할 경로 지정
        string directoryPath = Path.Combine(mvcFilePath, SubjectName); // 폴더 경로
        string savePath = Path.Combine(directoryPath, SubjectName + "_Arm_" + Direction + "_" + number.ToString() +".csv"); // 파일 경로
        ...
        Debug.Log($"MVC values saved to {savePath}");
